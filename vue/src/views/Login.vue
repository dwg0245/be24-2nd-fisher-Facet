...
const targetPath = (redirectPath && redirectPath !== 'undefined')
  ? redirectPath
  : '/users/user_information'; // 기본값

const loginWithKakao = () => {
  // 백엔드의 OAuth2 엔드포인트로 브라우저 주소 자체를 이동시킵니다.

  window.location.href = `${kakaoRedirectUri}?redirect_uri=${encodeURIComponent(targetPath)}`;
}

const loginWithGoogle = () => {
  window.location.href = `${googleRedirectUri}?redirect_uri=${encodeURIComponent(targetPath)}`;
}
</script>
    ...
    <div class="premium-card w-full max-w-[540px] rounded-3xl p-10 md:p-16">
      ...
      <div class="space-y-7">
        ...
        <div class="relative flex items-center justify-center py-4">
          ...
          <span class="flex-shrink mx-4 text-[10px] font-bold text-gray-300 uppercase tracking-widest">Or continue
            with</span>
          <div class="flex-grow border-t border-gray-100"></div>
        </div>

        <!-- Kakao Social Button -->
        <button id="kakao-login-btn" @click="loginWithKakao()"
          ...
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            <path
              d="M12 3c-4.97 0-9 3.181-9 7.105 0 2.53 1.704 4.753 4.318 6.007l-1.091 4.004c-.048.175.056.355.225.388.053.01.107.01.16-.002l4.711-3.118c.224.019.452.03.687.03 4.97 0 9-3.181 9-7.105S16.97 3 12 3z" />
          </svg>
          Login with Kakao
        </button>
