...
const calculatedPercent = computed(() => {
  if (!fundingDesc.value || !fundingDesc.value.targetPrice || fundingDesc.value.targetPrice === 0) return 0;
  const raised = fundingDesc.value.targetPrice;
  const goal = 10800000;
      ...
      <aside class="lg:col-span-4 lg:sticky lg:top-10 self-start space-y-6">
        <div class="border border-gray-100 rounded-md bg-white p-7 shadow-sm space-y-6">
          ...
          <div class="flex space-x-2 pt-1">
            <button class="flex-1 py-3 text-[11px] font-bold tracking-widest uppercase ghost-btn rounded-sm" style="border: 1px solid #d1d5db; color: #4b5563;">
